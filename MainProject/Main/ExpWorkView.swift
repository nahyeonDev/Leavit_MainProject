...
class ExpWorkView: UIViewController {
    ...
    @IBOutlet weak var goNextBtn: UIButton! //다음 버튼
    ...
    var keyNumber3 : String? //사용자 키 값 받아오기
    var cusKeyNum3 : String?
    ...
    @IBAction func goNext(_ sender: UIButton) {
        ...
        db.collection("가입세부정보").document(cusKeyNum3!).updateData(["경험 업무" : checkN])
        // navigation controller 로 화면 전환
          guard let keyVC = self.storyboard?.instantiateViewController(withIdentifier: "keywordCheckController")as? KeywordCheck else { return }
        keyVC.keyNumber4 = self.cusKeyNum3!
          self.navigationController?.pushViewController(keyVC, animated: true)
    }
    //카페 버튼
    @IBAction func clickCafe(_ sender: UIButton) {
        if(num[0] == 0){
            ...
            num[0] += 1
            goNextBtn.setImage(UIImage(named: "jnextBtn"), for: .normal)
            ...
            num[0] -= 1
            goNextBtn.setImage(UIImage(named: "xjnextBtn"), for: .normal)
            goNextBtn.isEnabled = false
            name[0] = ""
        }
    }
